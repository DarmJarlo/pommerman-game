...
class ForwardModel(object):
    ...
    def get_rewards(agents, game_type, step_count, max_steps):
        ...
        alive_agents = [num for num, agent in enumerate(agents) \
                        if agent.is_alive]
        diff = list(set(last_alive) - set(alive_agents))
        if game_type == constants.GameType.FFA:
            if len(alive_agents) == 1:
                ...
                #return [-1] * 4
                # Game is over from time. Everyone gets 0.
                return [0] * 4
            else:
            ...
            if any_lst_equal(alive_agents, [[0, 2], [0], [2]]):
                ...
                return [0] * 4
            """
            elif any_lst_equal(diff, [[1], [3]]) and any_lst_equal(alive_agents, [[0], [2]]):
                return [0.5, -0.5, 0.5, -0.5]
            elif any_lst_equal(diff, [[0], [2]]) and any_lst_equal(alive_agents, [[1], [3]]):
                return [-0.5, 0.5, -0.5, 0.5]
            """